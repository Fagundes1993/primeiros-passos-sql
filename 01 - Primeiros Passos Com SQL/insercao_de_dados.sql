-- Error Code: 1452. Cannot add or update a child row: a foreign key constraint fails (`company_constraints`.`employee`, CONSTRAINT `fk_employee` FOREIGN KEY (`Super_ssn`) REFERENCES `employee` (`Ssn`) ON DELETE SET NULL ON UPDATE CASCADE)
